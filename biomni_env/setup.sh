...
main() {
    ...
    has_data_dir=false
    has_restore_prefix=false

    [[ -d "$REPO_ROOT/data" ]] && has_data_dir=true
    [[ -n "${DEFAULT_S3_PREFIX:-}" || -n "${DEFAULT_COS_PREFIX:-}" ]] && has_restore_prefix=true

    if [[ "$has_data_dir" == false && "$has_restore_prefix" == true ]]; then
        ...
        if [[ -f "$REPO_ROOT/data_backup_restore.sh" ]]; then
            (cd "$REPO_ROOT" && bash ./data_backup_restore.sh restore) \
            || echo -e "${RED}Automatic restore failed. You can run it manually from ${REPO_ROOT}.${NC}"
        else
